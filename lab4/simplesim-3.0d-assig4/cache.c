	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  /* ECE552 Assignment 4 - BEGIN */
  if (prefetch_type >= 2) {
    int dim = prefetch_type == 2 ? DEFAULT_RPT_SIZE : prefetch_type;
    cp->rpt = (struct rpt_t*)calloc(dim, sizeof(struct rpt_t));
    int i;
    for (i = 0; i < dim; i++) {
      assert(cp->rpt[i].tag == 0);
      assert(cp->rpt[i].prev_addr == 0);
      assert(cp->rpt[i].stride == 0);
      assert(cp->rpt[i].state == 0);
      assert(cp->rpt[i].is_neg == 0);
    }
    if (prefetch_type == 2) {
      cp->prev_hist_addr = 0;
      for (i = 0; i < rdim; i++) {
...
void history_push_back (struct cache_t* cp, md_addr_t addr, md_addr_t index, md_addr_t tag) {
#if 1
  if (cp->tag[index] != tag) {
    cp->tag[index] = tag;
    cp->idx[index] = 1;
...
#endif
  if (cp->idx[index] >= cdim) {
    int i;
    for (i = 1; i < cdim; i++) {
      cp->ht[index][i - 1] = cp->ht[index][i];
    }
    cp->ht[index][cdim - 1] = addr;
    return;
  }
  cp->ht[index][cp->idx[index]] = addr;
  cp->idx[index]++;
}
...
int update_state (int curr_state, int stride_condition) {
  ...
  } else if (curr_state == transient) {
    if (stride_condition) {
      return steady;
    } else {
      return no_prediction;
    }
  } else if (curr_state == steady) {
    if (stride_condition) {
      return steady;
    } else {
      return initial;
    }
  } else if (curr_state == no_prediction) {
    if (stride_condition) {
      return transient;
    } else {
      return no_prediction;
