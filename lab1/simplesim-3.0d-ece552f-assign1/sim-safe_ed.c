...
static counter_t sim_num_RAW_hazard_q1_1;
static counter_t sim_num_RAW_hazard_q1_2;
static counter_t sim_num_RAW_hazard_q2;
static counter_t sim_num_RAW_hazard_q2_2;
static counter_t sim_num_RAW_hazard_q2_1;
/* ECE552 Assignment 1 - STATS COUNTERS - END */

/* ECE552 Assignment 1 - BEGIN CODE*/
int r_out[2], r_in[3];
static counter_t reg_ready[MD_TOTAL_REGS];
static counter_t reg_ready_2[MD_TOTAL_REGS];
...
//First Question
	{
	  ...
	  int cycle_distance = 0;
	    for (i = 0; i < 3; i++) {
	      	cycle_distance = reg_ready [r_in[i]] - sim_num_insn;
		if (r_in[i] != DNA && cycle_distance > 0) {
		    sim_num_RAW_hazard_q1++;
		    if (cycle_distance == 1) { sim_num_RAW_hazard_q1_1++; }
		    else if (cycle_distance == 2) { sim_num_RAW_hazard_q1_2++; }
		    break;
		  }
	      }
	}
...
//Second Question
        {
          ...
          int cycle_distance = 0;
            for (i = 0; i < 3; i++) {
                cycle_distance = reg_ready_2 [r_in[i]] - sim_num_insn;
                if (r_in[i] != DNA && cycle_distance > 0) {
		    if ((i == 0) && (MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_STORE))
                    ...
                    if (cycle_distance == 1) { sim_num_RAW_hazard_q2_1++; }
                    else if (cycle_distance == 2) { sim_num_RAW_hazard_q2_2++; }
                    break;
                  }
              }
        }
      ...
      if ((MD_OP_FLAGS(op) & F_MEM))) {
          if (r_out[0] != DNA)
              reg_ready_2[r_out[0]] = sim_num_insn + 3;
          if (r_out[1] != DNA)
              reg_ready_2[r_out[1]] = sim_num_insn + 3;
	}
      else {
	  if (r_out[0] != DNA)
              reg_ready_2[r_out[0]] = sim_num_insn + 2;
          if (r_out[1] != DNA)
              reg_ready_2[r_out[1]] = sim_num_insn + 2;
	}	
